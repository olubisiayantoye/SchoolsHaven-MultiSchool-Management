...
class Superadmin extends CI_Controller {
 ...
 $page_data['schools'] = $this->user_model->get_list('schools', array('enable_elearning'=> 1), '','', '', 'id', 'ASC');
        //var_dump($page_data['schools']); die();
    ...
    public function categories($param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add') {
            ...
            redirect(site_url('superadmin/categories'), 'refresh');
        }
        elseif ($param1 == "edit") {
       ...
       if ($_POST) {
            ...
            $page_data['categories'] = $this->crud_model->get_categories($param2,$school_id);
        }else{
    ...
    //////ADMINISTRATOR///////////////////////////////
        ...
        public function administrators($param1 = "", $param2 = "") {
        if ($this->session->userdata('superadmin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        if($param1 == "add"){
            $this->user_model->add_admin(true); // PROVIDING TRUE FOR administrator
            redirect(site_url('superadmin/administrators'), 'refresh');
        }
        elseif ($param1 == "edit") {
            $this->user_model->edit_user($param2);
            redirect(site_url('admin/administrators'), 'refresh');
        }
        elseif ($param1 == "delete") {
        ...
        $page_data['page_name'] = 'administrators';
        $page_data['page_title'] = get_phrase('administrator');
        $page_data['administrators'] = $this->user_model->get_administrator()->result_array();
        $this->load->view('backend/index', $page_data);
    }

    public function administrator_form($param1 = "", $param2 = "") {
        if ($this->session->userdata('superadmin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        if ($param1 == 'add_administrator_form') {
            $page_data['page_name'] = 'administrator_add';
            $page_data['page_title'] = get_phrase('administrator_add');
            $this->load->view('backend/index', $page_data);
        }
        elseif ($param1 == 'edit_administrator_form') {
            $page_data['page_name'] = 'administrator_edit';
            $page_data['user_id'] = $param2;
            $page_data['page_title'] = get_phrase('administrator_edit');
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function instructors($param1 = "", $param2 = "") {
        ...
        if($param1 == "add"){
            ...
            redirect(site_url('superadmin/instructors'), 'refresh');
        }
        elseif ($param1 == "edit") {
            $this->user_model->edit_user($param2);
    ...
    public function users($param1 = "", $param2 = "") {
        ...
        if ($param1 == "add") {
            ...
            redirect(site_url('superadmin/users'), 'refresh');
        }
        elseif ($param1 == "edit") {
            $this->user_model->edit_user($param2);
            redirect(site_url('superadmin/users'), 'refresh');
    ...
    public function enrol_student($param1 = "") {
        ...
        if ($param1 == 'enrol') {
            ...
            redirect(site_url('superadmin/enrol_history'), 'refresh');
        }
    ...
    public function payment_history_delete($param1 = "", $redirect_to = "") {
        ...
        redirect(site_url('superadmin/'.$redirect_to), 'refresh');
    }
    ...
    public function system_settings($param1 = "") {
        ...
        if ($param1 == 'system_update') {
            ...
            redirect(site_url('superadmin/system_settings'), 'refresh');
        }
    ...
    public function frontend_settings($param1 = "") {
        ...
        if ($param1 == 'frontend_update') {
            ...
            redirect(site_url('superadmin/frontend_settings'), 'refresh');
        }
    ...
    public function payment_settings($param1 = "") {
        ...
        if ($param1 == 'system_currency') {
            ...
            redirect(site_url('superadmin/payment_settings'), 'refresh');
        }
    ...
    public function smtp_settings($param1 = "") {
        ...
        if ($param1 == 'update') {
            ...
            redirect(site_url('superadmin/smtp_settings'), 'refresh');
        }
    ...
    public function instructor_settings($param1 = "") {
        ...
        if ($param1 == 'update') {
            ...
            redirect(site_url('superadmin/instructor_settings'), 'refresh');
        }
    ...
    public function theme_actions($action = "", $theme = "") {
        ...
        elseif ($action == 'remove') {
            ...
            redirect(site_url('superadmin/theme_settings'), 'refresh');
        }
    ...
    public function courses() {
        ...
        $page_data['selected_category_id']   = isset($_GET['school_id']) ? $_GET['school_id'] : "all";
    ...
    public function get_courses() {
        ...
        $courses = array();
        // Filter portion
        $filter_data['school_id']   = $this->input->post('school_id');
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $edit_this_course_url = site_url('superadmin/course_form/course_edit/'.$row->id);
                ...
                if ($row->status == 'active') {
                    ...
                    if ($row->user_id != $this->session->userdata('user_id')) {
                        ...
                        $course_status_changing_action = "confirm_modal('".site_url('superadmin/change_course_status_for_admin/pending/'.$row->id.'/'.$filter_data['selected_category_id'].'/'.$filter_data['selected_instructor_id'].'/'.$filter_data['selected_price'].'/'.$filter_data['selected_status'])."')";
                    }
                }else{
                    ...
                    if ($row->user_id != $this->session->userdata('user_id')) {
                        ...
                        $course_status_changing_action = "confirm_modal('".site_url('superadmin/change_course_status_for_admin/active/'.$row->id.'/'.$filter_data['selected_category_id'].'/'.$filter_data['selected_instructor_id'].'/'.$filter_data['selected_price'].'/'.$filter_data['selected_status'])."')";
                    }
                }



                $delete_course_url = "confirm_modal('".site_url('superadmin/course_actions/delete/'.$row->id)."')";
                ...
                $nestedData['title'] = '<strong><a href="'.site_url('superadmin/course_form/course_edit/'.$row->id).'">'.$row->title.'</a></strong><br>
    ...
    public function course_actions($param1 = "", $param2 = "") {
        ...
        if ($param1 == "add") {
            ...
            redirect(site_url('superadmin/course_form/course_edit/'.$course_id), 'refresh');
            ...
            redirect(site_url('superadmin/courses'), 'refresh');
        }
        elseif ($param1 == 'delete') {
    ...
    public function course_form($param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add_course') {
            ...
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $this->load->view('backend/superadmin/course_add_shortcut', $page_data);
    ...
    public function change_course_status($updated_status = "") {
        ...
        redirect(site_url('superadmin/courses?category_id='.$category_id.'&status='.$status.'&instructor_id='.$instructor_id.'&price='.$price), 'refresh');
    }
    ...
    public function sections($param1 = "", $param2 = "", $param3 = "") {
        ...
        redirect(site_url('superadmin/course_form/course_edit/'.$param1));
    }
    ...
    public function lessons($course_id = "", $param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add') {
            ...
            redirect('superadmin/course_form/course_edit/'.$course_id);
        }
        elseif ($param1 == 'edit') {
        ...
        elseif ($param1 == 'filter') {
            redirect('superadmin/lessons/'.$this->input->post('course_id'));
        }
    ...
    public function manage_language($param1 = '', $param2 = '', $param3 = ''){
        ...
        if ($param1 == 'add_language') {
            ...
            if ($language == 'n-a') {
                ...
                redirect(site_url('superadmin/manage_language'), 'refresh');
            }
    ...
    function message($param1 = 'message_home', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'send_new') {
            ...
            redirect(site_url('superadmin/message/message_read/' . $message_thread_code), 'refresh');
        }

        if ($param1 == 'send_reply') {
            $this->crud_model->send_reply_message($param2); //$param2 = message_thread_code
            $this->session->set_flashdata('flash_message', get_phrase('message_sent'));
            redirect(site_url('superadmin/message/message_read/' . $param2), 'refresh');
        }
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_profile_info') {
            $this->user_model->edit_user($param2);
            redirect(site_url('superadmin/manage_profile'), 'refresh');
        }
    ...
    public function paypal_checkout_for_instructor_revenue() {
        ...
        if ($payout_details['amount'] == $page_data['amount_to_pay'] && $payout_details['status'] == 0) {
            $this->load->view('backend/superadmin/paypal_checkout_for_instructor_revenue', $page_data);
        }else{
            $this->session->set_flashdata('error_message', get_phrase('invalid_payout_data'));
            redirect(site_url('superadmin/instructor_payout'), 'refresh');
        }
    ...
    public function stripe_checkout_for_instructor_revenue($payout_id) {
        ...
        if ($payout_details['amount'] > 0 && $payout_details['status'] == 0) {
            ...
            $this->load->view('backend/superadmin/stripe_checkout_for_instructor_revenue', $page_data);
        }else{
            $this->session->set_flashdata('error_message', get_phrase('invalid_payout_data'));
            redirect(site_url('superadmin/instructor_payout'), 'refresh');
        }
    ...
    public function quizes($course_id = "", $action = "", $quiz_id = "") {
        ...
        redirect(site_url('superadmin/course_form/course_edit/'.$course_id));
    }
    ...
    public function quiz_questions($quiz_id = "", $action = "", $question_id = "") {
        ...
        elseif ($action == 'delete') {
            ...
            redirect(site_url('superadmin/course_form/course_edit/'.$quiz_details['course_id']));
        }
    }
    ...
    public function install_theme($theme_to_install = '')
    {
        ...
        if (!in_array($theme_to_install, $uninstalled_themes)) {
            ...
            redirect(site_url('superadmin/theme_settings'));
        }
    ...
    public function addon($param1 = "", $param2 = "", $param3 = "") {
        ...
        if ($param1 == 'activate') {
            ...
            redirect(site_url('superadmin/addon'), 'refresh');
        }

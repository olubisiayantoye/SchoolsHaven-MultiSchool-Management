...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        $this->session->set_userdata('is_administrator', $row->is_administrator);
        $this->session->set_flashdata('flash_message', get_phrase('welcome').' '.$row->first_name.' '.$row->last_name);
        if($row->school_id){
        $school = sms_get_single('schools', array('status' => 1, 'id'=>$row->school_id));
    ...
    public function session_destroy() {
        ...
        if ($this->session->userdata('superadmin_login') == 1) {
            $this->session->unset_userdata('superadmin_login');

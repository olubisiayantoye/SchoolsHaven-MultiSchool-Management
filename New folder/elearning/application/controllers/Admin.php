...
class Admin extends CI_Controller {
    ...
    public function categories($param1 = "", $param2 = "") {
        ...
        if ($param1 == 'add') {
        $response = $this->crud_model->add_category($this->session->userdata('school_id'));
            if ($response) {
        ...
        $page_data['categories'] = $this->crud_model->get_categories($param2, $this->session->userdata('school_id'));
...
$check_sch = $this->crud_model->check_schoolsetting_by_id($school_id);
        if($check_sch->num_rows()==0){
          $this->crud_model->insert_schoolsetting_by_id($school_id);
        }


        if ($param1 == 'system_update') {
            $this->crud_model->update_schoolsystem_settings();
        ...
        $page_data['schoolsetting']  = $this->crud_model->check_schoolsetting_by_id($school_id)->row();
...
$school_id = $this->session->userdata('school_id');
$check_schoolsetting = $this->crud_model->front_schoolsetting_by_id($school_id);

        if($check_schoolsetting->num_rows()==0){
          $this->crud_model->insert_front_schoolsetting_by_id($school_id);
        }

        if ($param1 == 'frontend_update') {
            $this->crud_model->update_frontend_schoolsettings($school_id );
        ...
        if($param1 == 'recaptcha_update'){
            $this->crud_model->update_recaptcha_schoolsettings();
            $this->session->set_flashdata('flash_message', get_phrase('recaptcha_settings_updated'));
            redirect(site_url('admin/frontend_settings'), 'refresh');
        }

        if ($param1 == 'banner_image_update') {
            $this->crud_model->update_frontend_schoolbanner();
            $this->session->set_flashdata('flash_message', get_phrase('banner_image_update'));
            redirect(site_url('admin/frontend_settings'), 'refresh');
        }
        if ($param1 == 'light_logo') {
            $this->crud_model->update_light_schoollogo();
            $this->session->set_flashdata('flash_message', get_phrase('logo_updated'));
            redirect(site_url('admin/frontend_settings'), 'refresh');
        }
        if ($param1 == 'dark_logo') {
            $this->crud_model->update_dark_schoollogo();
            $this->session->set_flashdata('flash_message', get_phrase('logo_updated'));
            redirect(site_url('admin/frontend_settings'), 'refresh');
        }
        if ($param1 == 'small_logo') {
            $this->crud_model->update_small_schoollogo();
            $this->session->set_flashdata('flash_message', get_phrase('logo_updated'));
            redirect(site_url('admin/frontend_settings'), 'refresh');
        }
        if ($param1 == 'favicon') {
            $this->crud_model->update_schoolfavicon();
            $this->session->set_flashdata('flash_message', get_phrase('favicon_updated'));
            redirect(site_url('admin/frontend_settings'), 'refresh');
        }

$page_data['frontend_settings']  = $this->crud_model->front_schoolsetting_by_id($school_id)->row();
    ...
    public function course_actions($param1 = "", $param2 = "") {
        ...
        if ($param1 == "add") {
            $course_id = $this->crud_model->add_course($this->session->userdata('school_id'));

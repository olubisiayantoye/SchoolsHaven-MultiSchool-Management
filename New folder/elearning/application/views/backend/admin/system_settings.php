...
<div class="row">
    <div class="col-xl-7">
        ...
        <form class="required-form" action="<?php echo site_url('admin/system_settings/system_update'); ?>" method="post" enctype="multipart/form-data">
            <?php $ss = $schoolsetting; ?>
                        <div class="form-group">
                            <label for="system_name"><?php echo get_phrase('website_name'); ?><span class="required">*</span></label>
                            <input type="text" name = "system_name" id = "system_name" class="form-control" value="<?php if (!empty($ss->system_name)){echo $ss->system_name; }else{echo get_settings('system_name');}  ?>" required>
                        </div>

                        <div class="form-group">
                            <label for="system_title"><?php echo get_phrase('website_title'); ?><span class="required">*</span></label>
                            <input type="text" name = "system_title" id = "system_title" class="form-control" value="<?php if (!empty($ss->system_title)){echo $ss->system_title; }else{echo get_settings('system_title');}  ?>" required>
                        </div>

                        <div class="form-group">
                            <label for="website_keywords"><?php echo get_phrase('website_keywords'); ?></label>
                            <input type="text" class="form-control bootstrap-tag-input" id = "website_keywords" name="website_keywords" data-role="tagsinput" style="width: 100%;" value="<?php if (!empty($ss->website_keywords)){echo $ss->website_keywords; }else{ echo get_settings('website_keywords');}  ?>"/>
                        </div>

                        <div class="form-group">
                            <label for="website_description"><?php echo get_phrase('website_description'); ?></label>
                            <textarea name="website_description" id = "website_description" class="form-control" rows="5"><?php if (!empty($ss->website_description)){echo $ss->website_description; }else{ echo get_settings('website_description');}  ?></textarea>
                        </div>

                        <div class="form-group">
                            <label for="author"><?php echo get_phrase('author'); ?></label>
                            <input type="text" name = "author" id = "author" class="form-control" value="<?php if (!empty($ss->author)){echo $ss->author; }else{ echo get_settings('author');}  ?>">
                        </div>

                        <div class="form-group">
                            <label for="slogan"><?php echo get_phrase('slogan'); ?><span class="required">*</span></label>
                            <input type="text" name = "slogan" id = "slogan" class="form-control" value="<?php if (!empty($ss->slogan)){echo $ss->slogan; }else{ echo get_settings('slogan'); } ?>" required>
                        </div>

                        <div class="form-group">
                            <label for="system_email"><?php echo get_phrase('system_email'); ?><span class="required">*</span></label>
                            <input type="text" name = "system_email" id = "system_email" class="form-control" value="<?php if (!empty($ss->slogan)){echo $ss->slogan; }else{ echo get_settings('system_email'); } ?>" required>
                        </div>

                        <div class="form-group">
                            <label for="address"><?php echo get_phrase('address'); ?></label>
                            <textarea name="address" id = "address" class="form-control" rows="5"><?php if (!empty($ss->address)){echo $ss->address; }else{ echo get_settings('address');}  ?></textarea>
                        </div>

                        <div class="form-group">
                            <label for="phone"><?php echo get_phrase('phone'); ?></label>
                            <input type="text" name = "phone" id = "phone" class="form-control" value="<?php if (!empty($ss->phone)){echo $ss->phone; }else{ echo get_settings('phone');}  ?>">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name = "youtube_api_key" id = "youtube_api_key" class="form-control" value="<?php if (!empty($ss->youtube_api_key)){echo $ss->youtube_api_key; }else{ echo get_settings('youtube_api_key'); }  ?>" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name = "vimeo_api_key" id = "vimeo_api_key" class="form-control" value="<?php if (!empty($ss->youtube_api_key)){echo $ss->youtube_api_key; }else{ echo get_settings('vimeo_api_key');}  ?>" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" data-toggle="select2" name="language" id="language">
                                <?php foreach ($languages as $language): ?>
                                    <option value="<?php echo $language; ?>" <?php if(get_settings('language') == $language) echo 'selected'; ?>><?php if (!empty($ss->language)){echo ucfirst($ss->language); }else{ echo ucfirst($language);} ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name = "footer_text" id = "footer_text" class="form-control" value="<?php if (!empty($ss->footer_text)){echo $ss->footer_text; }else{ echo get_settings('footer_text');}  ?>">
                        </div>

                        <div class="form-group">
                            <label for="footer_link"><?php echo get_phrase('footer_link'); ?></label>
                            <input type="text" name = "footer_link" id = "footer_link" class="form-control" value="<?php if (!empty($ss->footer_link)){echo $ss->footer_link; }else{ echo get_settings('footer_link');}  ?>">
                        </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="col-lg-12">
                    ...
                    <form action="<?php echo site_url('updater/updateX'); ?>" method="post" enctype="multipart/form-data">

...
class User_model extends CI_Model {
    ...
    public function add_admin($is_administrator = false) {
...
$data['first_name'] = html_escape($this->input->post('first_name'));
    ...
    public function get_administrator($id = 0) {
        if ($id > 0) {
            return $this->db->get_where('users', array('id' => $id, 'is_administrator' => 1));
        }else{
            return $this->db->get_where('users', array('is_administrator' => 1));
        }
    }
    ...
    public function get_list($table_name, $index_array, $columns = null, $limit = null, $offset = 0, $order_field = null, $order_type = null) {
        ...
        if ($order_type) {
            $this->db2->order_by($order_field, $order_type);
        } else {
            $this->db2->order_by('id', 'DESC');
        }

        return $this->db2->get_where($table_name, $index_array)->result();
    }

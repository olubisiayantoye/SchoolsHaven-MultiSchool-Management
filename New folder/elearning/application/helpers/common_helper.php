...
if (! function_exists('get_frontend_schoolsettings')) {
    function get_frontend_schoolsettings($key = '') {
        
         if(defined('SITE_ID2')){
        $ci2 = & get_instance();
        $db2 = $ci2->load->database('sms', TRUE);
        $db2->where('id', SITE_ID2);
        $result = $db2->get('schools')->row();
        }else{
        $CI =&  get_instance();
        $CI->load->database();    
        $CI->db->where('key', $key);
...
if (! function_exists('get_bundle_validity')) {
    ...
    if (!function_exists('get_school_list')) {
                ...
                function get_school_list() {
                    ...
                    $db2 = $ci2->load->database('sms', TRUE);
                    $db2->select('S.*');
                    $db2->from('schools AS S');
                    $db2->where('S.status', 1);
                    $db2->where('S.enable_elearning', 1);
                    return $db2->get()->result();
                }
            }


    if (!function_exists('sms_get_single')) {
                    ////////////////

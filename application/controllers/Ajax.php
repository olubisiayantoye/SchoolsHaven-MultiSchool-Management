...
class Ajax extends My_Controller {
 ...
 public function get_section_by_class2() {
        ...
        if (!empty($sections)) {
            foreach ($sections as $obj) {
               // $selected = $section_id == $obj->id ? $select : '';
				 ...
				 $str .='<input   type="checkbox" name="section_id[]" value="' . $obj->id .'" />' .'Subject for Class Section: '. $obj->name . '<br />';
            }
        } 
    ...
    public function get_subject_by_class2() {
       
       
       $school_id = $this->input->post('school_id');
       $class_id = $this->input->post('class_id');
        
        $subjects = $this->ajax->get_list('subjects', array('status' => 1, 'school_id'=>$school_id, 'class_id' => $class_id), '', '', '', 'id', 'ASC');
        ...
        if (!empty($subjects)) {
            foreach ($subjects as $obj) {
               // $selected = $section_id == $obj->id ? $select : '';
                 ...
                 $str .='<input   type="checkbox" name="subject_id[]" value="' . $obj->id .'" />' .' - Exam Schedule for : '. $obj->name . '<br />';
            }
        } 
	 ...
	 public function get_section_by_class2xxxxx() {
       ...
       $class_id = $this->input->post('class_id');
        ...
        $str = '<option value="">--' . $this->lang->line('select') . '--</option>';
		 $str .= '<option value="0">  ' .'ALL SECTIONS'. '  </option>';
    ...
    $students = $this->ajax->get_student_list_by_section($school_id, $section_id, $academic_year_id);
        ...
        if (!empty($students)) {
            foreach ($students as $obj) {
                $selected = $student_id == $obj->id ? $select : '';
                $str .= '<option value="' . $obj->id . '" ' . $selected . '>' . $obj->surname . ' [' . $obj->roll_no . ']</option>';
            }
        }
    ...
    public function get_subject_by_class() {
        ...
        if(!empty($subjects)) {
            foreach ($subjects as $obj) {
               ...
               $str .= '<option value="' . $obj->id . '" ' . $selected . '>' . $obj->name .'['.$obj->code.']'. '</option>';
            }
        }
	  ...
	  public function get_subject_by_section_class() {
        ...
        //$subject_id = $this->input->post('subject_id');
            ...
            $subjects = $this->ajax->get_listsubject_section($school_id,$class_id,$section_id);
        print_r($subjects);
        $str = '<option value="">--' . $this->lang->line('select') . '--</option>';
  ...
  public function get_class_by_school() {
	 ...
	 public function get_school_year($year_id){
		 
		  $school = $this->ajax->get_single('academic_years',array('id'=>$year_id));         
            
		 return $school->school_id; 
		 }
	
	
	 public function get_class_by_year() {
     
         $year_id  = $this->input->post('year_id');
         $class_id  = $this->input->post('class_id');
		 $school_id = $this->get_school_year($year_id );
	...
	public function get_year_by_school() {
     
         $school_id  = $this->input->post('school_id');
       
         
        $years = $this->ajax->get_list('academic_years', array('status'=>1, 'school_id'=>$school_id), '','', '', 'id', 'ASC'); 
         
        $str = '<option value="">--' . $this->lang->line('select') . ' Sessin Year--</option>';
        $select = 'selected="selected"';
        if (!empty($years)) {
            foreach ($years as $obj) {   
                
                $selected = $school_id != $obj->school_id ? $select : '';
	 ...
	 public function generate_id_by_school() {
    ...
    public function update_result_comments(){
    ...
    ///////////////if NOT EXIST CREATE///////////////////////
        $result = $this->ajax->get_single('exam_results', $condition);
                    ...
                    if (empty($result)) {
                        ...
                        $data1['academic_year_id'] = $academic_year_id;
                        $data1['status'] = 1;
                        $data1['created_at'] = date('Y-m-d H:i:s');
                        $data1['created_by'] = logged_in_user_id();
                        $this->ajax->insert('exam_results', $data1);
                    }
  ///////////////////END//////////////////////////////////////////
    ...
    $this->ajax->update('exam_results', $data, $condition);
    ...
    public function newfrom_max_admission(){
    $current_year = date("Y");
    $school_id = $this->input->post('school_id');
    $this->db->select_max('admission_no');
    $this->db->where('school_id', $school_id);
    $this->db->like('admission_no', $current_year);

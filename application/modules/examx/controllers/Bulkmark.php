 ...
 * @type            : Class
 * @class name      : Bulk
 * @description     : Manage bulkmark students imformation of the school.  
...
class Bulkmark extends MY_Controller {
    ...
    * ********************************************************** */
    public function add() {
        ...
        if ($_POST) {            
            $status = $this->_get_posted_mark_data();
            if ($status) {                   

                create_log('Has been added Bulk Mark');
                success($this->lang->line('insert_success'));
                redirect('mark/index/'.$this->input->post('class_id'));
            } else {
                error($this->lang->line('insert_failed'));
                redirect('mark/bulk/add/');
            }            
        } 
 ...
 $this->data['classes'] = $this->mark->get_list('classes', array('status' => 1, 'school_id'=>$school_id), '', '', '', 'id', 'ASC');
        }else{ 
    ...
    /*****************Function _get_posted_mark_data**********************************
    * @type            : Function
    * @function name   : _get_posted_mark_data
    * @description     : Prepare "mark" user input data to save into database                  
    *                       
    * @param           : null
    * @return          : $data array(); value 
    * ********************************************************** */
    private function _get_posted_mark_data() {
		
		$sch_id = $this->input->post('school_id');
		
        $this->_upload_file($sch_id);

        $destination = 'assets/csv/bulk_'.$sch_id.'_uploaded_mark.csv';
        if (($handle = fopen($destination, "r")) !== FALSE) {
            ...
            while (($arr = fgetcsv($handle)) !== false) {
                ...
                //if ($arr[0] != '' && $arr[1] != '' && $arr[6] != '' && $arr[11] != '' && $arr[12] != '') {
                    ...
                    if ($this->mark->mark_duplicate_check($arr[0])) {
                        continue;
                    }
					...
					$data['admission_no'] = isset($arr[0]) ? $arr[0] : ''; //3.	Admission No:*
    ...
    * ********************************************************** */
    private function _upload_file($sch_id) {
        ...
        if ($file != "") {
            ...
            if ($ext == 'csv') {                 
                move_uploaded_file($_FILES['bulk_mark']['tmp_name'], $destination);  
            }
        } else {
            error($this->lang->line('insert_failed'));
            redirect('mark/bulk/add/');
    ...
    * @description     : save user info to users while create a new mark                  

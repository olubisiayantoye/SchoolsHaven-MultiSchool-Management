...
class Web_Model extends MY_Model {
    ...
    public function get_total_user($school_id){
		...
		public function get_active_theme($school_id ="")
		{
		
		//var_dump($this->session->userdata('front_school_id')); die();
		
		if($this->session->userdata('role_id') == SUPER_ADMIN){
         $this->db->select('*');
		 $this->db->from('system_admin'); 
		 $theme = $this->db->get()->row();
		return $theme->theme_template;
        }elseif(! is_null($this->session->userdata('front_school_id'))){
         $this->db->select('*');
		 $this->db->from('schools'); 
		 $this->db->where('id', $this->session->userdata('front_school_id'));
         $theme = $this->db->get()->row();
		 return $theme->theme_template;
        }
		else{

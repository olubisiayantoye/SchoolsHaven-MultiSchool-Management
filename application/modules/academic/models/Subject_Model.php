...
class Subject_Model extends MY_Model {
    ...
    function __construct() {
     ...
     public function get_subject_list($class_id = null , $school_id = null){
        ...
        //$this->db->select('S.*, SC.school_name, C.name AS class_name, SEC.name AS section_name, T.name AS teacher');
    ...
    function duplicate_check($school_id, $class_id,$name, $id = null ){   
	 ...
	 function duplicate_check_code($school_id,$ubject_code, $id = null ){   
        
        if($id){
            $this->db->where_not_in('id', $id);
        }
        //$this->db->where('class_id', $class_id);
		$this->db->where('code',$ubject_code);
        $this->db->where('school_id', $school_id);
        return $this->db->get('subjects')->num_rows();        
    }   

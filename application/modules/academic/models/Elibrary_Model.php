...
class Elibrary_Model extends MY_Model {
    ...
    public function get_elibrary(){        
        
        $this->db->select('D.*, S.school_name');
        $this->db->from('elibrarys AS D');
     ...
     public function get_elibrary_school($school_id){

        $this->db->select('*');
        $this->db->from('elibrarys');
    ...
    function duplicate_check($school_id, $name, $id = null ){           
        ...
        $this->db->where('school_id', $school_id);
        return $this->db->get('elibrarys')->num_rows();            
    }
}

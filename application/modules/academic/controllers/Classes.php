...
class Classes extends MY_Controller {
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
				$data2['cat_ass_test'] = implode(":", $this->input->post('cat_ass_test'));
				$data2['cont_mark'] = implode(":", $this->input->post('cont_mark'));
				$data2['abbreviation'] = implode(":",$this->input->post('abbreviation')); 
                $data1 = $this->_get_posted_class_data();
   ...
   {             
    ...
    private function _get_posted_class_data() {
		...
		$items[] = 'exam_mark';
        $items[] = 'numeric_name';
		$items[] = 'position_input';
        $items[] = 'position_format';        
        $items[] = 'note';

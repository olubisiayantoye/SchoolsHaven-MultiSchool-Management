...
class Subject extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null, $school_id = null) {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
			...
			$this->data['sections'] = $this->subject->get_list('sections', $condition, '','', '', 'id', 'ASC');
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
				$data1['section_id'] = implode(":",$this->input->post('section_id')); 
		
                $data2 = $this->_get_posted_subject_data();	
    ...
    * ********************************************************** */
    public function view($subject_id = null){
        ...
        $class_id = $this->data['subject']->class_id;
		$section_id = $this->data['subject']->section_id;
    ...
    * ********************************************************** */
    private function _prepare_subject_validation() {
        ...
        $this->form_validation->set_rules('code',$this->lang->line('code'), 'required|trim|callback_code');   
   ...
   {             
      if($this->input->post('id') == '')
      {   
          $subject = $this->subject->duplicate_check_code($this->input->post('school_id'),$this->input->post('code')); 
          if($subject){
         ...
         $subject = $this->subject->duplicate_check_code($this->input->post('school_id'),$this->input->post('code'), $this->input->post('id')); 
          if($subject){
                $this->form_validation->set_message('code', $this->lang->line('already_exist'));         
                return FALSE;
          } else {
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $sub_exist = $this->subject->data_exists('marks', 'subject_id', $id);
    //var_dump($sub_exist); die();
      if($sub_exist){
           error("Can not be deleted; The subject has been assigned to mark ");
          }else{

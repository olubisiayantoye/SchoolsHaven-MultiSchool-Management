...
/* * *****************Student.php**********************************
 * @product name    : Winnersworld Technology
 * @type            : Class
 * @class name      : Student
...
$this->load->model('Student_Model', 'student', true);            
    ...
    * @return          : null 
    * ********************************************************** */
    public function index($class_id = null, $school_id = null,$academic_year_id = null) {
    ...
    $this->data['filter_school_id'] = $school_id;
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
			...
			$school = $this->student->get_school_by_id( $this->session->userdata('school_id'));
			$academic_year_id = $school->academic_year_id;
			$year = $this->student->get_single('academic_years', array('id'=>$academic_year_id));
			//$this->data['runing_session'] = $year->session_year;

			$this->data['new_admission'] = $this->newfrom_max_admission($this->session->userdata('school_id'));
            $this->data['generate_id'] = $this->genetrate_student_id($this->session->userdata('school_id'));
            
          // START this is to ensure only in current academic year students be disenroll   
            $latestacademic_id = $this->student->get_single('academic_years', array('school_id'=>$this->session->userdata('school_id')))->id;
             if($latestacademic_id == $academic_year_id){
                $this->data['current_acada'] = 1;  
             }else{
               $this->data['current_acada'] = 0;    
             }
	   ...
	   $this->data['students'] = $this->student->get_student_list($class_id, $school_id, @$academic_year_id);
        $this->data['roles'] = $this->student->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        //var_dump($this->session->userdata('school_id')); die();
        
		
		$this->data['generateId'] = $this;
    ...
    * ********************************************************** */
    public function add() {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){  
			...
			$this->data['generate_id'] = $this->genetrate_student_id($this->session->userdata('school_id'));
            $this->data['new_admission'] = $this->newfrom_max_admission($this->session->userdata('school_id'));
            //var_dump($this->data['generate_id']); die();
        ...
        if ($_POST) {
            ...
            //UPDATE PASSWORD=========
                if($this->input->post('password')){
                  ...
                  $data22['password'] = md5($this->input->post('password'));
                $this->student->update('users', $data22, array('username' => $this->input->post('username')));
                }
    ...
    * ********************************************************** */
    ...
    public function get_single_student(){
	 ...
	 /*****************Function get_single_student card**********************************
     * @type            : Function
     * @function name   : get_single_student_card
     * @description     : "Load single student information" from database                  
     *                    to the user interface   
     * @param           : null
     * @return          : null 
     * ********************************************************** */
    public function get_single_student_card(){
        ...
        echo $this->load->view('get-single-student-card', $this->data);
    }
    
    
	 /*****************Function get_single_student**********************************
     * @type            : Function
     * @function name   : get_single_card_student
     * @description     : "Load single student information" from database                  
     *                    to the user interface   
     * @param           : null
     * @return          : null 
     * ********************************************************** */
    public function get_single_card_student(){
        ...
        echo $this->load->view('student_id', $this->data);
    }
    ...
    * ********************************************************** */
    private function _prepare_student_validation() {
        ...
        $this->form_validation->set_rules('guardian_id', $this->lang->line('guardian'), 'trim');
    ...
    * ********************************************************** */
    private function _get_posted_student_data() {
       ...
       /* $addmm = $this->genetrate_student_admin($school_id);
        //var_dump(isset($admission_no)); die();

        if(empty($admission_no)){
           $data['admission_no'] = $addmm; 
        }else{
        $items[] = 'admission_no';
        } */
    ...
    * ********************************************************** */
    ...
    $file = $_FILES['photo'];
    $query = $this->upload_model->post_student_image_upload($file);
    ...
    * ********************************************************** */
    public function delete($id = null) {
      ...
      /*****************Function delist disenrollment*****************
    * @type            : Function
    * @function name   : disenrollment delist
    * @description     : delist "Student" data from database                  
    *                     also delist all relational data
    *                     and unlink student photo from server   
    * @param           : $id integer value
    * @return          : null 
    * ********************************************************** */
    public function disenrollment() {
       ...
       // var_dump($student_id); die();
        ...
        $enrollstudent_id = $this->student->get_single('enrollments', array('school_id'=>$school_id,'student_id'=>$student_id, 'academic_year_id' => $academic_year_id))->id; 
        //var_dump($enrollstudent_id); die();       
     
        
        if (!empty($enrollstudent_id)) {
            
            
            // delete student data
        $this->student->delete('enrollments', array('id' =>  $enrollstudent_id));
            ...
            success('disenrollment successful');
            create_log('student with id '.$student_id.'and enroll ID'.$enrollstudent_id.' successfully disenroll');
        } else {
            error('disenrollment failed');
        }

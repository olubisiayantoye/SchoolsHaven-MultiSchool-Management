...
class Mark_Model extends MY_Model {
    ...
    public function get_student_list($school_id = null, $exam_id = null, $class_id = null, $section_id = null, $subject_id = null, $academic_year_id = null){
        ...
        $this->db->select('ES.*, E.roll_no, S.admission_no,  C.name AS class_name, S.id AS student_id, S.name AS student_name, S.photo, S.surname AS surname,  S.phone');
		...
		public function get_subject($school_id,$class_id,$subject_id){
	             $this->db->select('*');
		...
		public function get_academic($school_id,$academic_year_id){
	             $this->db->select('*');
...
public function student_exists_mark($school_id,$exam_id,$class_id,$section_id,$subject_id,$academic_year_id,$student_id)
		{
			...
			$query = $this->db->get('marks');
				...
				public function student_id($admission_no){
				...
				public function subjectcode_id($subjectcode,$class_id,$school_id){
			
				$this->db->where('school_id', $school_id);
				$this->db->where('class_id', $class_id);
				$this->db->where('code', $subjectcode); 
				$query = $this->db->get('subjects');
				return $query->row()->id;
				
				}
				
			public function update_change_insection($school_id,$exam_id,$class_id,$academic_year_id,$subject_id,$section_id,$student_id){
				$data = array(
				...
				public function update_change_insection_attendance($school_id,$exam_id,$class_id,$academic_year_id,$subject_id,$section_id,$student_id){
				$data = array(
				'section_id' => $section_id,
				'is_attend' => 1
				);
				...
				return $this->db->update('exam_attendances', $data);

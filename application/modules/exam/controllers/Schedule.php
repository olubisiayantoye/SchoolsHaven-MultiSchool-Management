           ...
           // var_dump($this->form_validation->run()); die();
            if (isset($_POST['subject_id'])) {
             ...
             foreach($_POST['subject_id'] as $value){
                $start_timeo = $this->input->post('start_time');
                $end_timeo = $this->input->post('end_time');

                
            $start_time = date('h:i A',strtotime('+'.$aa.' hour +20 minutes',strtotime($start_timeo)));
            $end_time = date('h:i A',strtotime('+'.$bb.' hour +20 minutes',strtotime($end_timeo)));
            ...
            $data['created_by'] = logged_in_user_id();
             ...
             if ($this->schedule->duplicate_check($data['school_id'], $school->academic_year_id, $data['exam_id'], $data['class_id'], $value)) {
                        continue;
                    }

                    
             if ($this->schedule->insert('exam_schedules', $data)) {
                    success($this->lang->line('insert_success'));

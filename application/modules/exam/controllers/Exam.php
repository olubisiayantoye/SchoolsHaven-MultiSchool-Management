...
class Exam extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_exam_validation() {
        ...
        $this->form_validation->set_rules('term', $this->lang->line('exam_term') . '' . $this->lang->line('term'), 'trim|required|callback_term');
...
/*****************Function term**********************************
    * @type            : Function
    * @function name   : title
    * @description     : Unique check for "Exam term " data/value
    * @param           : null
    * @return          : boolean true/false 
    * ********************************************************** */ 
    public function term() {
        ...
        if ($this->input->post('id') == '') {
            $exam = $this->exam->duplicate_check($school_id, $school->academic_year_id, $this->input->post('term'));
            if ($exam) {
                $this->form_validation->set_message('term', $this->lang->line('already_exist_term'));
                return FALSE;
            } else {
                return TRUE;
            }
        } else if ($this->input->post('id') != '') {
            $exam = $this->exam->duplicate_check($school_id, $school->academic_year_id, $this->input->post('term'), $this->input->post('id'));
            if ($exam) {
                $this->form_validation->set_message('term', $this->lang->line('already_exist'));
                return FALSE;
            } else {
                return TRUE;
            }

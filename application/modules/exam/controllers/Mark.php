...
$section_idd = $this->db->get_where('enrollments' , array('student_id' => $obj->student_id, 'academic_year_id' => $school->academic_year_id))->row()->section_id; 
            // var_dump($section_idd ); die();   
    $this->mark->update_change_insection($school_id,$exam_id,$class_id,$school->academic_year_id,$subject_id,$section_idd, $obj->student_id );
    $this->mark->update_change_insection_attendance($school_id,$exam_id,$class_id,$school->academic_year_id,$subject_id,$section_idd, $obj->student_id);
...
$this->data['grades'] = $this->mark->get_list('grades', array('status' => 1, 'school_id'=>$school_id), '', '', '', 'id', 'ASC');
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){
            ...
            $academic_year_id = $school->academic_year_id;
             // START this is to ensure only in current academic year marks be manage   
            $latestacademic_id = $this->mark->get_single('academic_years', array('school_id'=>$this->session->userdata('school_id')))->id;
             if($latestacademic_id == $academic_year_id){
                $this->data['current_acada'] = 1;  
             }else{
               $this->data['current_acada'] = 0;    
             }
           // END this is to ensure only in current academic year marks be manage 
        ...
        if ($_POST) {
            ...
            if (!empty($_POST['students'])) {
                ...
                'subject_id' => $subject_id
                    ...
                    // ################--ALL TOTAL MARK--#######
                   $data['exam_total_mark'] = (int)$_POST['exam_total_mark'];
                    //$data['obtain_total_mark'] = $_POST['obtain_total_mark'][$value];
                  @(int)$data['obtain_total_mark'] = @(int)$data['written_obtain'] + @(int)$data['tutorial_obtain'] + @(int)$data['practical_obtain']+ @(int)$data['viva_obtain']+ @$data['exam_obtain'] ;
...
$data['section_id'] =  $this->db->get_where('enrollments' , array('student_id' => $value,'academic_year_id' => $academic_year_id))->row()->section_id;

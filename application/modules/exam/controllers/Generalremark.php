...
class Generalremark extends MY_Controller {
  ...
  $this->data['school'] = $this->generalremark->get_school_by_id($school_id);	
  $this->data['students'] = $this->generalremark->get_student_list($school_id, $class_id, $section_id, $school->academic_year_id);
    ...
    function generate_generalremark__csv($school_id,$xam_id,$class_id, $section_id,$academic_year_id)
    {
       ...
       $school = $this->generalremark->get_school_by_id($school_id);
        ...
        if(empty($school->psycho_1)){
            $psycho_1 = 'Handwritting';
        }else{
            $psycho_1 = $school->psycho_1;
        }

        if(empty($school->psycho_2)){
            $psycho_2 = 'Games';
        }else{
            $psycho_2 = $school->psycho_2;
        }

        if(empty($school->psycho_3)){
            $psycho_3 = 'Games';
        }else{
            $psycho_3 = $school->psycho_3;
        }

        if(empty($school->psycho_4)){
            $psycho_4 = 'Leave Blank';
        }else{
            $psycho_4 = $school->psycho_4;
        }

        if(empty($school->psycho_5)){
            $psycho_5 = 'Leave Blank';
        }else{
            $psycho_5 = $school->psycho_5;
        }

        if(empty($school->affective_1)){
            $affective_1 = 'Speak/Fluency';
        }else{
            $affective_1 = $school->affective_1;
        }

        if(empty($school->affective_2)){
            $affective_2 = 'Punctual/neatness';
        }else{
            $affective_2 = $school->affective_2;
        }

        if(empty($school->affective_3)){
            $affective_3 = 'Attentiveness';
        }else{
            $affective_3 = $school->affective_3;
        }

        if(empty($school->affective_4)){
            $affective_4 = 'Leave Blank';
        }else{
            $affective_4 = $school->affective_4;
        }

        if(empty($school->affective_5)){
            $affective_5 = 'Leave Blank';
        }else{
            $affective_5 = $school->affective_5;
        }

        

   /* $speak_fluency='Speak/Fluency';
    $punctua_neat ='Punctual/neatness';
    $Attentiveness ='Attentiveness';
    $games = 'Games';
    $handwritting = 'Handwritting';
    $music_fineart = 'Music/fineart'; */
 ...
 $line   = array('Student ID','Fullname', $psycho_1 , $psycho_2 , $psycho_3 , $affective_1 , $affective_2 , $affective_3 , $psycho_4 , $affective_4 , $psycho_5 , $affective_5 ,'Teachers Comment','General Comment','Conduct','Attendance','Position');
        fputcsv($file, $line, ',');
        ...
        if ($_POST) {
            ...
            if (!empty($_POST['students'])) {
                ...
                foreach ($_POST['students'] as $key => $value) {
                	...
                	$data['psycho_1'] = $_POST['psycho_1'][$value];
                    $data['psycho_2'] = $_POST['psycho_2'][$value];
                    $data['psycho_3'] = $_POST['psycho_3'][$value];
                    $data['psycho_4'] = @$_POST['psycho_4'][$value];
                    $data['psycho_5'] = @$_POST['psycho_5'][$value];
                
                    $data['affective_1'] = $_POST['affective_1'][$value];
                    $data['affective_2'] = $_POST['affective_2'][$value];
                    $data['affective_3'] = $_POST['affective_3'][$value];
                    $data['affective_4'] = @$_POST['affective_4'][$value];
                    $data['affective_5'] = @$_POST['affective_5'][$value];
 ...
 public function upload() {
	 ...
	 public function _uploadmark($school_id,$exam_id, $class_id,$section_id,$academic_year_id) {
        ...
        if (($handle = fopen($destination, "r")) !== FALSE) {
			...
			$data['psycho_1 '] = isset($arr[2]) ? $arr[2] : '';
			$data['psycho_2'] = isset($arr[3]) ? $arr[3] : '';
			$data['psycho_3'] = isset($arr[4]) ? $arr[4] : '';	
			$data['affective_1'] = isset($arr[5]) ? $arr[5] : '';	
			$data['affective_2'] = isset($arr[6]) ? $arr[6] : '';	
			$data['affective_3'] = isset($arr[7]) ? $arr[7] : '';
            $data['psycho_4'] = isset($arr[8]) ? $arr[8] : '';
            $data['affective_4'] = isset($arr[9]) ? $arr[9] : '';
            $data['psycho_5'] = isset($arr[10]) ? $arr[10] : '';
            $data['affective_5'] = isset($arr[11]) ? $arr[11] : '';

			$data['teacher_comment'] = isset($arr[12]) ? $arr[12] : '';	
			$data['general_comment'] = isset($arr[13]) ? $arr[13] : '';
			$data['conduct'] = isset($arr[14]) ? $arr[14] : '';
			$data['attendance'] = isset($arr[15]) ? $arr[15] : '';		
			$data['class_position'] = isset($arr[16]) ? $arr[16] : '';

...
class Ajax_Model extends MY_Model {
    ...
    public function get_student_list_by_section($school_id = null, $section_id = null, $academic_year_id = null){
        
        $school = $this->get_school_by_id($school_id);
        
        $this->db->select('E.roll_no, S.surname, S.id');
        $this->db->from('enrollments AS E');        
        $this->db->join('students AS S', 'S.id = E.student_id', 'left');
        $this->db->where('E.academic_year_id', $academic_year_id); 
    ...
    public function get_user_list($school_id, $type) {
	 ...
	 public function get_listsubject_section($school_id,$class_id,$section_id){
		  
		  $this->db->select('*');
          $this->db->from('subjects');
		  $this->db->like('section_id', $section_id);
		  $this->db->where('school_id', $school_id);
		  $this->db->where('class_id', $class_id);
		  if($this->session->userdata('role_id') == TEACHER){
			  $this->db->where('teacher_id', $this->session->userdata('profile_id'));
			  }

...
$CI->load->model('Ajax_Model');
	...
	$all_exam  		= 	$this->db->get_where('exams' , array('academic_year_id' => $academic_year_id,'school_id' => $school_id))->result_array();
	//var_dump($all_exam); die();
	
	
	//find score by sch_id, exam_id,subj_id, class_id, section_id
...
<table width="100%" border="0" align="center">
  ...
  <tr>
    <td align="center">
      <?php //echo $exam_name;?>
      <?php echo $running_year." ALL TERMS ANNUAL SUMMARY REPORT" ;?>
      </td>
  </tr>
</table>
...
<table id="customers" ;="" style="width:100%; border-collapse:collapse;border: 1px solid #ccc; margin-top: 10px;" border="1">
                   <thead>
                    ...
                    <tbody id="fn_mark">   
                                                                                                                                                                         <tr>
                                    <td>1</td>
                                    <td>CULTURAL AND CREATIVE ARTS / MUSIC</td>
	...
	<table id="customers"; style="width:100%; border-collapse:collapse;border: 1px solid #ccc; margin-top: 10px;" border="1">
                   <thead>
                        <tr>
                            ...
                            <th>Exam</th>    
                            <th colspan="9">SUMMARY OF TERM'S' WORK</th>                           
                        </tr>
                        <tr>
                         <?php
						    if (isset($class_ass) && !empty($class_ass)) {
						 ...
						 // var_dump($class_ass->abbreviation); die();
						 @$totalcat += $cclmm[$xx];
                           echo "<th>".$this->lang->line('obtain').$cclmm[$xx]."%"."</th>";
                            ...
                            <!--th>Mark</th-->                                            
                            <th>Obtain <?php echo 100-$totalcat; ?>%</th>
                            <!--th>Total AA</th-->
                            
                            <th>3RD Total <?php echo "100%"; ?></th>
                            <!--header for smarr exam ---> 
                            <?php foreach ($all_exam as $obj_exam) {?>
                           
                            <th><?php echo explode(' ',trim($obj_exam['title']))[0]; ?> Total <?php echo "100%"; ?></th>
                      
                            <?php }  ?> 
                             <!end--header for smarr exam --->                                             
                            <th>Mark</th>
                            <th>&nbsp;</th>
                            <th>&nbsp;</th>
                            <th>&nbsp;</th>
    ...
    $CI->checkmyresult_model->delete_duplicate_mark($school_id, $exam_id, $section_id, $student_id, $obj->subject_id, $academic_year_id);                        
                 ...
                 <!--practical #--Third Continuous Assessment Test--|||||||||||||||||||||||||||||||--> 
                   ...
                   <!--practical #--Fourth Continuous Assessment Test|||||||||||||||||||end   |-->
                                    ...
                                    <!--td><?php //echo $obj->exam_total_mark; ?>AA</td-->
                            ...
                            echo "<td>".$sum_score."</td>"; 
                             ...
                             <!end--header for smarr exam ---> 
                                   ...
                                   <?php //$result = $CI->checkmyresult_model->get_grade($obj->obtain_total_mark,$class_id,$school_id ) ; echo $result["point"]; ?>  
                                    ...
                                    <td><?php echo $position; ?></td>             <td><?php echo $obj->exam_obtain; ?></td>
                                    <td><?php echo $obj->exam_total_mark; ?></td>
                            <td><?php //echo $obj->exam_obtain; ?>
                            <?php
...
<table  style="width:100%; border-collapse:collapse;border: 1px solid #ccc; margin-top: 10px;" border="0" >
  <tr>
    ...
    <td></td>
    <td>&nbsp;</td>
    <td><strong> <?php echo "Student's Average Mark";?></strong> : 
	        <?php @$average_mark = (@$total_marks/(@$count-1));
			echo number_format(@$average_mark,2); 
			//$percent = (@$total_marks*100) / (@$exam_total_mark);
	          
			   //@$grade_point = number_format(@$percent,2);
...
//Example Usage
	...
	if($position_format ==2){
		...
		$grade_name = $CI->checkmyresult_model->get_grade(@$average_mark2,$class_id,$school_id ) ; echo "Grade ".$grade_name["name"];

	 	//echo "Grade ".$myGrade->name;
		//var_dump($myGrade); die();
	}
...
<table id="customers" ;="" style="width:100%; border-collapse:collapse;border: 1px solid #ccc; margin-top: 10px;" border="1">
                   <thead>
                    ...
                    <!--loop summary term work <th>THIRD</th>--->
                        ...
                        <?php foreach ($all_exam as $obj_exam) {?>
            <th><?php echo explode(' ',trim($obj_exam['title']))[0]; ?> </th>
                        <?php }  ?> 
                        ...
                        <!--loop summary term work--<td>ok</td>-->
                           ...
                           $summaexam_total += $sum_score;
                            $numexa++;
                              ...
                              ?> 
                         <!--end loop summary term work --->
                        <td><?php  $summaexam_total2 = $summaexam_total/$numexa; 
                        echo $summaexam_total2;
                        ?></td>
                                    <td>
                                     <?php
							 if (isset($summaexam_total2) && !empty($summaexam_total2)) {
$result2 = $CI->checkmyresult_model->get_grade($summaexam_total,$class_id,$school_id ) ; echo $result2["name"]; 
                                 }

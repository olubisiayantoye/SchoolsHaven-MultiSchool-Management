...
if (!function_exists('get_student_position')) {
    ...
    function get_student_position($school_id, $academic_year_id, $class_id, $student_id, $section_id = null) {
        ...
        // Build the condition
        $condition = "school_id = $school_id AND academic_year_id = $academic_year_id AND class_id = $class_id";
        if ($section_id) {
            $condition .= " AND section_id = $section_id";
        }
        
        // SQL query to get rank using subquery
        $sql = "SELECT id, avg_grade_point, total_obtain_mark,
                       (SELECT COUNT(*) + 1 
                        FROM final_results AS fr 
                        WHERE (fr.avg_grade_point + fr.total_obtain_mark) > (final_results.avg_grade_point + final_results.total_obtain_mark)
                          AND fr.school_id = $school_id
                          AND fr.academic_year_id = $academic_year_id
                          AND fr.class_id = $class_id
                          " . ($section_id ? "AND fr.section_id = $section_id" : "") . "
                       ) AS student_rank 
                FROM final_results 
                WHERE $condition AND student_id = $student_id
                LIMIT 1";
        
        // Execute query and fetch result
        $result = $ci->db->query($sql)->row();
        
        // Determine the rank and add ordinal suffix
        $rank = !empty($result) ? $result->student_rank : null;
...
if (!function_exists('get_lowet_height_mark')) {
    ...
    function get_lowet_height_mark($school_id, $exam_id, $class_id, $section_id, $subject_id) {
        ...
        $ci->db->from('marks AS M');  
        //adrin no zero
        $ci->db->where('M.obtain_total_mark IS NOT NULL');
        $ci->db->where('M.obtain_total_mark > 0');
        //end
        $ci->db->where('M.school_id', $school_id);
        $ci->db->where('M.exam_id', $exam_id);
        $ci->db->where('M.class_id', $class_id);
...
if (!function_exists('get_position_in_subject')) {
    ...
    function get_position_in_subject($school_id, $exam_id, $class_id, $section_id, $subject_id, $mark) {
        ...
        $sql = "SELECT id, obtain_total_mark, FIND_IN_SET(obtain_total_mark, (
                    SELECT GROUP_CONCAT(obtain_total_mark ORDER BY obtain_total_mark DESC)
                    FROM marks
                    WHERE school_id = $school_id AND exam_id = $exam_id AND class_id = $class_id AND section_id = $section_id AND subject_id = $subject_id
                )) AS position_rank
                FROM marks
                WHERE school_id = $school_id AND exam_id = $exam_id AND class_id = $class_id AND section_id = $section_id AND subject_id = $subject_id AND obtain_total_mark = $mark";
...
if (!function_exists('get_position_in_subject')) {
    ...
    function get_position_in_subject($school_id, $exam_id, $class_id, $section_id, $subject_id, $mark) {
        ...
        // Revised SQL query to calculate rank using a subquery
        $sql = "SELECT id, obtain_total_mark, 
                       (SELECT COUNT(*) + 1 
                        FROM marks AS m 
                        WHERE m.school_id = $school_id 
                          ...
                          AND m.obtain_total_mark > obtain_total_mark
                       ) AS position_rank 
                FROM marks
        ...
        // Check if the result is valid and mark is greater than zero
        if (!$result || $mark == 0) {
            return '--';
        }
        ...
        // Adding ordinal suffix based on rank
        if ($rank == 1) {
            return $rank . 'st';
        } elseif ($rank == 2) {
            return $rank . 'nd';
...
/////////////////////////////////////////////////////////////////////////////////////
/* if (!function_exists('find_position_in_class')) {

    function find_position_in_class($school_id, $exam_id, $class_id, $mark) {
        
        
        $ci = & get_instance();
        $sql = "SELECT id, obtain_total_mark, FIND_IN_SET( obtain_total_mark,(
                SELECT GROUP_CONCAT( obtain_total_mark  ORDER BY obtain_total_mark DESC ) 
                FROM marks WHERE  school_id = $school_id AND exam_id = $exam_id AND class_id = $class_id))
                AS rank 
                FROM marks
                WHERE school_id = $school_id AND exam_id = $exam_id AND class_id = $class_id AND obtain_total_mark = $mark"; 
        
        $rank1 =  $ci->db->query($sql)->row();
		$rank = $rank1->rank; 
...
if (!function_exists('find_position_in_section')) {

    function find_position_in_section($school_id, $exam_id, $class_id, $section_id, $mark) {
        
        
        $ci = & get_instance();
      
				
					$sql =	"SELECT id, name, score, FIND_IN_SET( score, (
...
if (!function_exists('find_position_in_class')) {
    ...
    function find_position_in_class($school_id, $exam_id, $class_id, $mark) {
        ...
        // Revised query to calculate rank based on higher marks
        $sql = "SELECT id, obtain_total_mark, 
                       (SELECT COUNT(*) + 1 
                        FROM marks AS m 
                        WHERE m.school_id = $school_id 
...
if (!function_exists('find_position_in_section')) {
    ...
    function find_position_in_section($school_id, $exam_id, $class_id, $section_id, $mark) {
        ...
        // Revised query to calculate rank based on higher scores within the section
        $sql = "SELECT id, score, 
                       (SELECT COUNT(*) + 1 
                        FROM scores AS s 
        ...
        // Fetching result and rank
        $rank_result = $ci->db->query($sql)->row();
...
function student_class_position_manual($exam_id,$class_id,$student_id){
		...
		$ci = & get_instance();
		 $query = $ci->db->get_where('exam_comment', array('student_id' => $student_id ,'class_id' => $class_id , 'exam_id' => $exam_id))->row();
		...
		$ci->db->select('student_id, SUM(obtain_total_mark) as total');
			// $ci->db->select_sum('mark_obtained');
			 $ci->db->from('marks');
             $ci->db->where('school_id' , $school_id);
			 $ci->db->where('exam_id' , $exam_id);
             $ci->db->where('class_id' , $class_id);
           
			// $ci->db->where('mark_obtained >' ,$total_mark_obtained);
...
if (!function_exists('get_exam_wise_markt')) {
    ...
    function get_exam_wise_markt($school_id, $exam_id, $class_id, $section_id, $student_id) {
        ...
        $select = 'SUM(M.written_mark) AS written_mark,
				   ...
				   SUM(M.exam_mark) AS exam_mark,
                   SUM(M.exam_obtain) AS exam_obtain,
...
if (!function_exists('get_position_in_exam')) {
    ...
    function get_position_in_exam($school_id, $exam_id, $class_id, $section_id, $mark) {
        ...
        // Revised query to calculate rank using a subquery
        $sql = "SELECT id, total_obtain_mark, 
                       (SELECT COUNT(*) + 1 
                        FROM exam_results AS er 
                        WHERE er.school_id = $school_id 
                          AND er.exam_id = $exam_id 
                          AND er.class_id = $class_id 
                          AND er.section_id = $section_id 
                          AND er.total_obtain_mark > total_obtain_mark
                       ) AS rank 
                FROM exam_results
                WHERE school_id = $school_id 
                  AND exam_id = $exam_id 
        ...
        // Fetch the result and determine the rank
        $result = $ci->db->query($sql)->row();
...
if (!function_exists('get_position_in_class')) {
    ...
    function get_position_in_class($school_id, $academic_year_id, $class_id, $student_id) {
        ...
        // Updated SQL query with alias 'student_rank' instead of 'rank'
        $sql = "SELECT id, avg_grade_point, total_obtain_mark,
                       (SELECT COUNT(*) + 1 
                        FROM final_results AS fr 
                        WHERE fr.school_id = $school_id 
                          AND fr.academic_year_id = $academic_year_id 
                          AND fr.class_id = $class_id 
                          AND (fr.avg_grade_point + fr.total_obtain_mark) > (avg_grade_point + total_obtain_mark)
                       ) AS student_rank 
                FROM final_results
                WHERE school_id = $school_id 
        ...
        // Fetch the result and determine the rank
        $result = $ci->db->query($sql)->row();
        $rank = $result ? $result->student_rank : null;
